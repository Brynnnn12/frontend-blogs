...
const BlogPage = () => {
  ...
  // Use useMemo untuk filter comments agar tidak re-render terus
  const postComments = useMemo(() => {
    if (!comments || !Array.isArray(comments) || !currentPost?.id) {
      return [];
    }
    return comments.filter((comment) => comment.postId === currentPost.id);
  ...
  // Load comments hanya sekali saat component mount
  useEffect(() => {
    dispatch(getComments());
  }, [dispatch]);

  // Load comments saat post ID berubah (bukan saat currentPost object berubah)
  useEffect(() => {
    if (currentPost?.id) {
      dispatch(getComments());
    }
  }, [currentPost?.id, dispatch]); // Hanya depend pada ID, bukan object
  ...
  const handleSubmit = async (values, { resetForm }) => {
    if (!currentPost?.id) {
      console.error("Post ID tidak tersedia");
      return;
    }
